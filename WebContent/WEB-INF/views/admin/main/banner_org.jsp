		...
		<tbody>
			...
			%>
				...
				<tr>
					...
					<td align="left"><input type="text" style="width:350px;" name="dispSubj" value='<%=data.getdispSubj()%>'></td>
				...
				<tr >
					<td align="left"><textarea name="dispLink" style="width:98%;height:60px;"><%=data.getdispLink()%></textarea>
					 *순위 <select name="dispRanking" id="dispRanking" style="width:80px;" onchange="ranking(<%=//data.getdispId()%>,this);">
						<option value="">선택</option>
					    <option value="1" <% //if(data.getdispRanking().equals("1")){ %>selected<%}%>>1번째</option>
						<option value="2" <%// if(data.getdispRanking().equals("2")){ %>selected<%}%>>2번째</option>
						<option value="3" <% //if(data.getdispRanking().equals("3")){ %>selected<%}%>>3번째</option>
					</select>
					&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:choice_sel('<%=//data.getdispId()%>','U','<%=//type%>');">위</a> <a href="javascript:choice_sel('<%=//data.getdispId()%>','D','<%=//type%>');">아래</a>
					</td>	
				</tr>
...
function ranking(id,str) {
	var str = str.value;
	
	$.ajax({
		url : 'updateBannerRanking.do' ,
		type : 'post' ,
		data : {dispId:id,dispRanking:str},
		dataType : 'json' ,
		error : function(){
			alert('서버와 통신도중 에러가 발생하였습니다.');
		},
