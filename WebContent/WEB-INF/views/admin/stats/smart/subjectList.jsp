...
<div class="btn-toolbar">
	<a class="btn btn-primary" href="gradeList.do">학교급별</a>
	<a class="btn btn-danger" href="#">교과별</button>
	<a class="btn btn-primary" href="bookList.do">도서별</a>
</div>
...
<div>
	...
	<tr>
		...
		<td style="border:1px solid #eee;" colspan="3">
			<input type="radio" name="findFlag" value="H" <% if( findFlag.equals("H")){ %>checked<% } %>/> 표제어
			<input type="radio" name="findFlag" value="P" <% if( findFlag.equals("P")){ %>checked<% } %>/> 백과플러스
			<input type="radio" name="findFlag" value="C" <% if( findFlag.equals("C")){ %>checked<% } %>/> 인물사건캘린더
			<input type="radio" name="findFlag" value="E" <% if( findFlag.equals("E")){ %>checked<% } %>/> 역사전시관
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td style="border:1px solid #eee;" colspan="3">
			<select name="findCategory" class="category" style="width:100px;margin-right:10px;float:left;">
				<option value="B001" <% if( findCategory.equals("B001")){ %>selected<% } %>>초등</option>
				<option value="B002" <% if( findCategory.equals("B002")){ %>selected<% } %>>중등</option>
				<option value="B003" <% if( findCategory.equals("B003")){ %>selected<% } %>>고등</option>
			</select>
			<a class="btn btn-primary" onclick="goSearch();">검색</a>
